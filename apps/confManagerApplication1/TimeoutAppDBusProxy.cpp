...
TimeoutAppDBusProxy::TimeoutAppDBusProxy(std::shared_ptr<TimeoutApp> app,
	sdbus::IConnection& connection, const std::string& service_name, const std::string& object_path)
	: app_(app), BaseConfigurationProxy(connection, service_name, object_path) {
		registerProxy();
}

TimeoutAppDBusProxy::~TimeoutAppDBusProxy() { unregisterProxy(); }
...
void TimeoutAppDBusProxy::onConfigurationChanged(const std::map<std::string, sdbus::Variant>& configuration) {
	...
	auto a = app_.lock();
	if (!a) throw std::runtime_error("App is not running");

	auto timeoutIt = configuration.find("Timeout");
	uint32_t timeout_ms = a->getTimeoutMs();
	if (timeoutIt != configuration.end() && 
	timeoutIt->second.containsValueOfType<uint32_t>())
	...
	if (timeoutPhraseIt != configuration.end() &&
	timeoutPhraseIt->second.containsValueOfType<std::string>())
